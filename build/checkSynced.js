...
const checkSynced = async () => {
    ...
    for (const { id } of notSynced) {
        try {
            const inAgents = await (0, variables_1.db)('js_agents').where('fk_cb_id', id);
            const inTasks = await (0, variables_1.db)('js_tasks').where('fk_cb_id', id);
            const inConfig = await (0, variables_1.db)('js_config').where('fk_cb_id', id);
