...
const addTxDetail = async () => {
    const rowsNeedingUpdate = await (0, variables_1.db)('js_transactions').select('hash', 'id').where('is_complete', false).limit(6);
    ...
    for (const { id, hash } of rowsNeedingUpdate) {
        ...
        try {
            ...
            await (0, variables_1.db)('js_transactions').where('id', id)
                .update({
